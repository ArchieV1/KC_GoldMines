...
public class IMCPort : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            ULogger.Log("InterModComm", $"TimeOut {timeOut.ID}");
    ...
    private void ReceiveMessage(object[] data)
    {
        try
        {
            ...
            else
            {
                ...
                if (!receiveListeners.ContainsKey(message.Name))
                {
                    ...
                    ULogger.Log("InterModComm", $"Unhandled request {message}");
        ...
        {
            ULogger.Log("InterModComm", $"Failed to process message: {ex.Message}");
            ULogger.Log("InterModComm", $"Failed to process message: {ex.StackTrace}");
        }
    }
    ...
    {
        ULogger.Log("IntermodComm", "Sending IMC Message");
        ULogger.Log("InterModComm", $"Target: {_target}, message: {message}");
        ...
        ULogger.Log("InterModComm", "Sent IMC Message");
    }
